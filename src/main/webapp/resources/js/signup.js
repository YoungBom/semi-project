(() => {
  ...
  function buildEmail() {
    ...
    const full = local + domain; // ex) loser123 + @naver.com
  ...
  btnCheck && btnCheck.addEventListener('click', async () => {
    const id = ((userId && userId.value) || '').trim().toLowerCase();
    if (!id) {
      if (idChecked) idChecked.value = 'false';
  ...
  const openSimpleModal = (msg) => openSuccessModal(msg);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // 서버가 HTML로 forward한 경우도 성공으로 간주하여 로그인 화면으로 이동
